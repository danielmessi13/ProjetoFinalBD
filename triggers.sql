...
begin 
	if tg_op = 'INSERT' or tg_op = 'UPDATE' then
			...
			if new.cod_tipo_conta not in (select cod_tipo_conta from tipo_conta) then
...
create or replace function controle_cliente()
returns trigger as $$
  begin
...
create or replace function controle_deposito()
returns trigger as $$
  begin
    if tg_op = 'INSERT' or tg_op = 'UPDATE' then
      ...
      if new.saldo > ((select avg(valor) from partes_movimentacao where numero_conta = new.numero_conta) + 500) then
